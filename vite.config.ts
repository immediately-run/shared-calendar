...
//
// `__APP_DEV__` is how app code learns it is running under plain `vite dev`
// (no host): `import.meta` is a parse error in the sandbox transpiler, so a
// Vite `define`d global is the only safe switch. On immediately.run the global
// is undefined, which src/lib/store.ts reads as "not dev".
